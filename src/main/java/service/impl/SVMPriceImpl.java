...
public class SVMPriceImpl implements SVMPriceService {
    ...
    public List<SVM> predictByOCHL_List(String stockId) {
        ...
        List<String> time_series=timeseries(stockId);
        List<DayK> dayKs=train("2016",stockId);
        ...
        for(int i=duration-1;i>=0;i--) {
            List<DayK> dayKs2 = predict(time_series.get(i),stockId);//i=0表示最近的一天开始往前推days天
    ...
    public double predictByOCHL_Tomorrow(String stockId) {
        ...
        List<DayK> dayKs2=predict(null,stockId);
        svm_node[] svm_nodes=new svm_node[4*days];//用于预测的数据是一个模为4*days的向量
        for(int j=0;j<days;j++)
        {
            svm_node pa0 = new svm_node();
        ...
        return dayKs.get(index).getOpen()-dayKs.get(index-1).getOpen()+dayKs2.get(0).getOpen();//打印预测后一天开盘价
    }

    /**训练集，目前用2016年一年的数据来训练*/
    public List<DayK> train(String year,String stockId) {
        List<DayK> dayKList = dayKDAO.getPriceTrainData(year,stockId);
        return dayKList;
    }
    ...
    public List<DayK> predict(String time,String stockId) {
        ...
        if(time==null||time.isEmpty())
        {
            ...
            dayKList = dayKDAO.getPricePreditDataList(time,stockId);
        }
        return dayKList;
    }
    ...
    {
        List<String> time_series = dayKDAO.getDates(stockId);
        return time_series;
    }
}
